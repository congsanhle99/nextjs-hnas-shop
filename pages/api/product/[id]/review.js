  ...
  try {
    ...
    if (product) {
      ...
      if (exist) {
        ...
        updateProduct.rating = updateProduct.reviews.reduce((a, r) => r.rating + a, 0) / updateProduct.reviews.length;

        await updateProduct.save();
        await updateProduct.populate("reviews.reviewBy");
        await db.disconnectDb();
        return res.status(200).json({ reviews: updateProduct.reviews.reverse() });
      } else {
        const review = {
          reviewBy: req.user,
